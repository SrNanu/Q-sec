...
from datos.models import SimulationSession, User  # noqa: F401
...
def create_app(config=None, crear_tablas=True):
    ...
    if crear_tablas:
        with app.app_context():
            db.create_all()
            # create_all() no agrega columnas a tablas existentes: sin esto, una
            # base local creada antes de un cambio de modelo dejaría de funcionar
            agregar_columnas_faltantes(db.engine, db.metadata)
