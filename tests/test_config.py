...
class TestUrlDeBaseDeDatos:
    """Render y Heroku entregan postgres://, que SQLAlchemy 2.0 no reconoce."""

    def test_corrige_el_esquema_de_render_y_heroku(self):
        assert normalizar_url('postgres://u:p@host/db') == 'postgresql://u:p@host/db'

    def test_sqlalchemy_acepta_la_url_corregida(self):
        from sqlalchemy.engine import make_url
        from sqlalchemy.exc import NoSuchModuleError

        with pytest.raises(NoSuchModuleError):
            make_url('postgres://u:p@host/db').get_dialect()
        assert make_url(normalizar_url('postgres://u:p@host/db')).get_dialect().name == 'postgresql'

    def test_no_toca_una_url_ya_correcta(self):
        url = 'postgresql://u:p@host/db'
        assert normalizar_url(url) == url

    def test_no_fuerza_un_driver(self):
        """Quien use psycopg2 o psycopg 3 tiene que poder seguir eligiendo."""
    ...
    def test_solo_reemplaza_el_prefijo(self):
        ...
        salida = normalizar_url('postgres://user:postgres://x@host/db')
        assert salida.startswith('postgresql://')
...
class TestFactoryDeAplicacion:
    ...
    def test_registra_todas_las_rutas(self):
        ...
        for esperado in ('home', 'login', 'register', 'logout', 'dashboard',
                         'simulator', 'history', 'simulation_result',
